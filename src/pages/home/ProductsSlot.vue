<template>

    <div class="ml-10 flex w-72 mx-auto overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
        <div class="w-1/3 bg-cover" style="background-image: url('https://images.unsplash.com/photo-1494726161322-5360d4d0eeae?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=334&q=80')"></div>

        <div class="w-2/3 p-4 md:p-4">
            <h1 @click="toDetail" class="text-2xl font-bold text-gray-800 dark:text-white">{{ product.productName }}</h1>

            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{{ product.productDescp }}</p>

            

            <div class="flex justify-between mt-3 item-center">
                <h1 class="text-lg font-bold text-gray-700 dark:text-gray-200 md:text-xl">${{ product.productPrice }}</h1>
                <button class="px-2 py-1 text-xs font-bold text-white uppercase transition-colors duration-200 transform bg-gray-800 rounded dark:bg-gray-700 hover:bg-gray-700 dark:hover:bg-gray-600 focus:outline-none focus:bg-gray-700 dark:focus:bg-gray-600">Add to Cart</button>
            </div>
        </div>
        <router-view></router-view>
    </div>
</template>

<script>
import router from '../../router'
export default {
  name: 'ProductsSlot',
  props: [
    'product'
  ],
  methods: {
    toDetail: function () {
      const id =  this.product.productID
      router.push({
        path: `/products/${id}`
      })
    }
  }
}
</script>
